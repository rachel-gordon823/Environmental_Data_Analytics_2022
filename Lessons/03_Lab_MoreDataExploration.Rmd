...
matrix3
<<<<<<< HEAD
matrix1[4, ] #only prints a row, specifically row 4, and all the columns of row 4
matrix1[ , 3] #only prints a column
=======
...
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #only rows 2:4 and columns 1:3
...
e <- c("red", "white", "red", NA) # numeric vector
f <- c(TRUE, TRUE, TRUE, FALSE) # character vector
...
```{r}
high.temps <- c(49,55,37,38,42,37,45,50,52,51)
high.temps
low.temps <-c (30,31,19,24,27,18,24,29,28,37)
low.temps
```
...
high.temps.c <-c(fahrenheit.to.celsius(high.temps))
high.temps.c
low.temps.c <-c(fahrenheit.to.celsius(low.temps))
low.temps.c
```
...
```{r}
DurhamTemps <- data.frame(HighF=high.temps,HighC=high.temps.c,LowF=low.temps,LowC=low.temps.c)
DurhamTemps
```
