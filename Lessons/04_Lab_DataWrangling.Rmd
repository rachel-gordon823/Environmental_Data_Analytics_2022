...
```{r}
#before choosing rows, important to know what the class of the row is
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth)

# dplyr filtering; only want when depth = 0
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)

# Choose multiple conditions to filter; summary shows we have most observations with Paul Lake and Peter Lake
...
class(NTL.phys.data$year4) #stored as an integer
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999)
# what code do you need to use, based on the class of the variable? Need to use == code
...
NTL.phys.data.Tues.1990thru1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))
...
```{r}
#Building a pipe, start with initial data, then filter by lake name, then select column lakename, then choose between lakedate and then between sampledate:temperature
...
NTL.phys.data.processed3 <-
  NTL.phys.data.processed2 %>%
  filter(depth==0) %>%
  filter(!is.na(temperature_C)) %>% #doing this to not get errors if there are blanks
  summarise(mean = mean(temperature_C)) #this will summarise with just the mean
