...
#applying additional asthetics argument, going beyond default binwidth
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#setting number of bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 10)

#intrdocuing scle_x_continuous, forces x to go from 0-500
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#this is a different variable, showing the change in skew
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#adding a line to the histogram
ggplot(USGS.flow.data) +
...
#coloring the values by the mean approval, can set color to different categorical data
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#parsing data into two data sets with the y variable code
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#taking different slices of data, slice the data by group and cut_width command, this is analagous to histograms, instead of looking at counts, look at distributions along the slice of the data
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#another way to just look and visualize data
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#seeing the relationship between the two, as we get more volume, gage height doesn't increase as much, makes sense because stream in shape of bowl, stream flow goes out
ggplot(USGS.flow.data) +
  geom_point(aes(x = discharge.mean, y = gage.height.mean))

#datetime is hybrid between categorical and continuous, showing mean discharge at different times. Plot shows that there is big chunk in time when there is no data, if x is purely categorical, not going to be as interesting because all data will really just fall on one line
ggplot(USGS.flow.data) +
