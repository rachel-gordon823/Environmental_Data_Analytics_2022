...
shapiro.test((EPAair.subsample$Ozone)) #tells us it's not normal
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)
#qq plot shows some extreme, which is why Shapiro said it's not normal

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample
#testing for less than or equal to 50
#p-value is less than .05, reject the null hypothesis, which is mu >= 50
...
- Going to check for normality

```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018]) #checking for normality
...
# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) #regressing over categorical variable (year) which has two levels, 2018 and 2019
O3.twosample
...
```{r}
#Exercise 2:
ozone.year <- ggplot(EPAair, aes(x=Ozone, fill = as.factor(Year)))+ #changing year to a factor, use fill instead of color for transparent
  geom_density(alpha = 0.5)+ #changing density
  scale_fill_manual(values = c("darkgreen", "darkblue"))+
 labs(x= "Ozone AQI value", y = "Density", fill = "") #making sure legend has no name
print(ozone.year)

ozone.year.box <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone))+
  geom_boxplot()+
  stat_summary(geom = "text", fun.y = max, vjust = -1, size=4, label =c("b", "a"))+ #everything in stat.summary is for the labels
labs(x="", y = "Ozone AQI Value")
print(ozone.year.box) 

ozone.violin <- ggplot(EPAair, aes(x=as.factor(Year), y = Ozone))+
...
O3.PM2.5 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(O3.PM2.5)

#rsquare 0.1316, want to add more variables to push the r-square up
#p-value 2.2e-16

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
PM2.5byO3 <- ggplot(EPAair, aes(x=Ozone, y = PM2.5))+
  geom_point()+
  geom_smooth(method="lm")
  print(PM2.5byO3)

  #when you look at scatter plot, you see that the points are not close together with respect to blue line.Which means we are probably missing some variables
...
OzoneALL.regression <-lm(data=EPAair, formula = PM2.5 ~ Ozone + Year + Month + meanLat + meanLng)
step(OzoneALL.regression)
...
OzoneBest.regression <- lm(data=EPAair, PM2.5~Ozone + Month + meanLng + meanLat)
