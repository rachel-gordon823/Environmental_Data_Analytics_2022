...
str(NTL.phys.data) #data type of each column
dim(NTL.phys.data)
...
```{r}
class(NTL.phys.data$lakeid) #can only compare classes that are the same
class(NTL.phys.data$depth)

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]#filtering for rows that have depth = 0
...
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake")#selecting all of those that don't equal Peter Lake or Paul Lake, excluding all of the other lakes that are not what you want
...
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth))#need to specify if want descending instead of ascending
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C)#selecting lakename, sampledate, depth, and temperature_C
...
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32)#changing the temperature to Celsius
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) #adding a new column 'month'

# reorder columns to put month with the rest of the date variables
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments)#use select function to change order of columns, moving month after daynum
...
#function first and last, gives you first and last of the column
```
