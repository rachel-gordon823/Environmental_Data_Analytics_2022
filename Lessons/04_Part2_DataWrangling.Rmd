...
```{r, message = FALSE}
getwd()
library(plyr) #package good for getting data set the way you want it to be
library(tidyverse)
library(lubridate)
...
summary(NTL.nutrient.data.PeterPaul$lakename) #summary still shows info from the larger dataset
...
summary(NTL.nutrient.data.PeterPaul$lakename)#now only Paul and Peter Lake show up
...
count(NTL.nutrient.data.PeterPaul.gathered, nutrient)#specify column to count by, asking R to count nutrients
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration)#take names from column nutrient and values from concentration

# Split components of cells into multiple columns
# Opposite of 'separate' is 'unite'
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, sampledate, c("Y", "m", "d"))#want three values separate in each column
...
The Niwot Ridge litter dataset, when downloaded from NEON, comes packaged with each month as a different .csv file. If we want to analyze the dataset as a single data frame, we need to combine each of these files. Can bind elements by row.
...
Litter.2019 <- rbind(Litter.June2016, Litter.July2016, Litter.August2016)#binding all the months
```
...
NTL.PeterPaul.summaries <- 
  ...
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter na for temp, tn_ug, tp_ug
