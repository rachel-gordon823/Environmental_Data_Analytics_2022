...
Rainfall_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% #pasting all the elements together
...
#geom line is preferred way to look at time series plots over time
...
fmonth <- month(first(BRA_complete$Date)) #extracting first month of date column
fyear <- year(first(BRA_complete$Date)) # extracting first year of data column

  BR_Rain_ts <- ts(BRA_complete$Rainfall, start = c(fmonth,fyear), frequency=12)
  BR_Temp_ts <- ts(BRA_complete$Temperature, start = c(fmonth,fyear), frequency=12)
```
