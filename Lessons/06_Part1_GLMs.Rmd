...
editor_options: 
  chunk_output_type: console #output going to console
---
...
#on average, residuals are always going to be 0
#for linear regression, always check the residuals
...
#p value is telling you the significance of the regression

#if r-squared is close to 0, look at more variables
...
>  significant negative correlation between irradiance and depth
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing plot screen into 2x2
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #removing outliers
  geom_point() 
print(irradiancebydepth) 
#clearly no linear relationship when looking at graph
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000)#taking the log to see if it has a linear relationship, remove 0's to take log and get rid of outlier
...
corrplot(PeterPaulCorr, method = "ellipse") #correlation plot matrix, color and shape gives you strength and sign of correlation
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #considering all 5 columns of dataset as explanatory variables

#every time you remove a variable and your AIC goes up, that means that variable should remain
#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)

#after the none, keep all of those variables
