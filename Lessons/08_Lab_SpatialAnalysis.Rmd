...
ggplot() +
  geom_sf(data=huc8_sf, aes(fill=ACRES))
...
neus <- st_read('./Data/Spatial/NCHUC8.shp') %>%
...
wind_data <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12'='factor')) %>%
  mutate(HUC_8 = substr(HUC_12,8)) %>%
  group_by()
...
selected_HUC5 <- huc8_sf_utm[triangle_sf,]
mapview(selected_HUCS)
