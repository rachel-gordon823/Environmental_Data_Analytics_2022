...
```{r}
<<<<<<< HEAD
#expression at x = 0, y = .025
#if you have na's on dataset, always going to get a warning when building a plot
=======
library(ggplot2)
...
PeterPaulplot.Ex2 <-
  ggplot(PeterPaul.chem.nutrients,
         aes(x=sampledate, y=temperature_C))+
  geom_point(aes(color=depth), size=0.5) +
  ylab("Temperature in Celsius")+
  scale_color_viridis(option = "magma",
                      direction = -1) + #changing to 1 will reorder the color
  scale_x_date(
    date_breaks = "5 years", date_labels = 
      "%Y")
...
ggplot(EPAair, aes(x = PM2.5, y=Ozone))+
  geom_point(aes(color=mean_lat), alpha = .5) +
